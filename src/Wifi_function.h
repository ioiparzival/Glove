...
void Transmit_string_wifi() // Передаем строку по wifi машинке
{
  if(sens_val[1] > 0 && sens_val[0] == 0 && sens_val[2] == 0)
    master.print("A" + String(sens_val[1]) + " B" + String(sens_val[1]) + " C"
                 + String(sens_val[1]) + " D" + String(sens_val[1]) + " ");
  else if(sens_val[2] > 0 && sens_val[0] == 0 && sens_val[1] == 0)
    master.print("A" + String(-sens_val[2]) + " B" + String(-sens_val[2])
                 + " C" + String(-sens_val[2]) + " D" + String(-sens_val[2])
                 + " ");
  else if(sens_val[1] > 0 && sens_val[0] > 0 && sens_val[2] == 0)
    master.print("A" + String(-sens_val[1] * 0.7) + " B"
                 + String(sens_val[1] * 0.7) + " C"
                 + String(-sens_val[1] * 0.7) + " D"
                 + String(sens_val[1] * 0.7) + " ");
  else if(sens_val[0] > 0 && sens_val[2] > 0 && sens_val[1] == 0)
    master.print("A" + String(sens_val[0] * 0.7) + " B"
                 + String(-sens_val[0] * 0.7) + " C"
                 + String(sens_val[0] * 0.7) + " D"
                 + String(-sens_val[0] * 0.7) + " ");
  else
